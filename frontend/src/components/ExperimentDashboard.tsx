...
export function ExperimentDashboard({
  ...
  const generate = () =>
    run(
      () => api.generate(experimentId, 3),
      () => setMessage('Generated 3 new challenger variants.'),
    );

  const evaluate = () =>
    run(
      () => api.evaluate(experimentId),
      (r) => {
            ...
            return (
              <tr key={v.id} className={isLeading ? 'lead-row' : ''}>
                <td>
                  <span
                    className="vkey"
                    style={{ background: KEY_COLORS[i % KEY_COLORS.length] }}
                  >
                    {LETTERS[i] ?? '•'}
                  </span>
                ...
                <td>
                  {isLeading ? (
                    ...
                    <span className="muted">
                      {v.isControl ? 'Baseline' : 'Challenger'}
                    </span>
                  )}
                </td>
              </tr>
        ...
        <div className="win-banner">
          <div>
            ...
            <span className="win-text">
              <b>
                Variant{' '}
                {LETTERS[ordered.findIndex((v) => v.id === sig.bestChallengerId)]} is
                winning!
              </b>
